...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_data_loader):
            ...
            text_features, text_pooled, video_pool, loss = self.model(data, is_train=True)

            output = sim_matrix_training(text_features, video_pool, self.pooling_type)
            loss_otv = self.loss(output, self.model.clip.logit_scale)
            output = sim_matrix_training(text_pooled, video_pool, self.pooling_type)
            loss_etv = self.loss(output, self.model.clip.logit_scale)
            loss_tv = loss_otv + loss_etv

            loss_all = loss_tv + loss
    ...
    def _valid_epoch_step(self, epoch, step, num_steps):
        ...
        with torch.no_grad():
            ...
            # Based on T-Mass [https://github.com/Jiamian-Wang/T-MASS-text-video-retrieval]
            text_embeds_pooled = torch.zeros(size=(text_embeds.shape[0], text_embeds.shape[0], text_embeds.shape[1]))
            for idx_vid, single_tid in tqdm(enumerate(text_embeds)):
                text_embeds_pooled[idx_vid, :, :] = text_embeds  # [N, N ,D]

            del text_embeds, vid_embeds
            gc.collect()

            text_embeds_per_video_id, vid_embeds_pooled_per_video_id = generate_embeds_per_video_id(text_embeds_pooled, vid_embeds_pooled, all_vid_ids, self.pooling_type)
